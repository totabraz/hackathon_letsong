	...
	const filterParceiros = (name = "") => {
		...
		if (name !== "") {
			tempParceiros = initial_parceiros.filter((parceiro) => {
				...
				console.log("parceiro.name", parceiro.name);
				console.log(
					"parceiro.name.includes(name)",
					simplestText(parceiro.name).includes(simplestText(name))
				);

				return simplestText(parceiro.name).includes(simplestText(name));
			});
			// if (tempParceiros.length > 0) tempParceiros = tempParceiros[0];
		}
